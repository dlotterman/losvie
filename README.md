...
1. **Build**: Repackge an Enterprise Linux Live ISO with the contents of [squashfs_inserts](squashfs_inserts/)
...
3. **Install**: Use a [libvirt](https://libvirt.org/) VM as a sort of "outside-in" OOB, providing the CD-ROM, KVM and hardware + bootdisk passthrough to the installer
...
3. No access to untagged broadcast domains across servers (so no arp / DHCP -> PXE)
...
losvie started as an "exploration of a bad idea for fun", and instead of being a bad idea, turned out to be a perpetually useful tool in the toolbelt for a small number of Operators. 

Credit to [enkelprifti98/metal-isometric-xepa](https://github.com/enkelprifti98/metal-isometric-xepa) as most adjacent inspiration.
